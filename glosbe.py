...
def getData(theWord, orglang, destlang):
    ...
    r = requests.get(
        'http://glosbe.com/gapi/translate?from=%s&dest=%s&format=json&phrase=%s&pretty=true' % (orglang, destlang, theWord))
    try:
...
r2 = requests.get(
    'http://glosbe.com/gapi/translate?from=%s&dest=%s&format=json&tm=true&phrase=%s&pretty=true' % (orglang, destlang, theWord))
try:
    ...
    if len(phr) == 0:
        ...
        for item in range(0, len(phr)):
            try:
                ll = len(
                    str(phr[item]['phrase']['text']).encode('UTF-8').rstrip(' '))
                if ll > maxl:
                    maxl = ll
            except:
                ll = 10
        ...
        try:
            ...
            if data['tuc'][item]['phrase']['text'] <> ' ':
                ...
                for ite in range(0, len(data['examples'])):
                    ...
                    ss = data['examples'][ite]['second'].encode(
                        'UTF-8').replace('<strong class="keyword">', '')
                    ss = ss.replace(
                        '</strong>', '').replace('#', '').replace('|', '')

                    sf = data['examples'][ite]['first'].replace(
